...
- **eject 전 정리**: 각 볼륨에서 `dot_clean` 실행 후 macOS 메타데이터 찌꺼기(`._*`, `.DS_Store`) 삭제 — 회사 윈도우에서 지저분하게 보이지 않게. `.Spotlight-V100` 등 폴더는 안 건드림.
- **즉시 인지**: 누른 순간 `Pop` 소리. 블로킹되는 `diskutil eject`는 소리가 먼저 나도록
  다음 틱(`doAfter 0`)에 실행한다.
- **결과 피드백**: eject 후 **우상단 토스트**로 표시 (`hs.alert`는 화면 중앙 고정이라
  위치를 못 옮겨서 `hs.canvas`로 우상단에 직접 그린다)
  - 성공: "⏏︎ 꺼냄: … — 안전하게 뽑으세요"
